...
**更新** 本版本新增了基于 CommunityToolkit MVVM 的现代化插件开发示例，展示如何使用 ObservableObject、RelayCommand 等现代C#特性进行插件开发。
...
**更新** 新增的M3U8下载器插件还依赖 CliWrap 库用于FFmpeg调用，展示了如何在插件中集成第三方库。
...
- **更新** M3U8下载器插件采用了并发下载策略，通过 SemaphoreSlim 控制最大并发数，平衡下载速度与系统资源占用。
...
**更新** 新的M3U8下载器插件展示了现代C#开发的最佳实践，包括使用CommunityToolkit MVVM的ObservableObject、RelayCommand等特性，为开发者提供了现代化的插件开发参考案例。
...
- 测试与调试
  ...
  - **更新** 对于M3U8下载器插件，建议使用真实M3U8测试链接进行端到端测试。
- 部署
  - 将插件输出复制到宿主应用的插件目录，确保插件清单与依赖满足要求。
  - 使用共享库提供的安装管理器进行安装与卸载操作。
  - **更新** 部署M3U8下载器插件时，确保系统已安装FFmpeg可执行文件。