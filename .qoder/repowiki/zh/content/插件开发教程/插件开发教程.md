...
- [M3u8DownloaderPage.axaml](file://plugins/Avalonia.Plugin.Downloader/Pages/M3u8DownloaderPage.axaml)
...
- [Avalonia.Plugin.Downloader.csproj](file://plugins/Avalonia.Plugin.Downloader/Avalonia.Plugin.Downloader.csproj)
...
**更新** 本版本新增了基于 CommunityToolkit MVVM 的现代化插件开发示例，展示如何使用 ObservableObject、RelayCommand 等现代C#特性进行插件开发。特别关注M3U8下载插件作为现代化MVVM的完整实现案例。
...
DOWN["Avalonia.Plugin.Downloader<br/>DownloaderPlugin.cs"]
end
subgraph "共享库"
IF1["IPlugin.cs"]
...
### 组件四：M3U8下载器插件（现代化MVVM示例）
这是最复杂的插件示例，展示了如何使用 CommunityToolkit MVVM 的现代C#特性进行插件开发。

#### 插件架构概览
M3U8下载器插件采用分层架构设计，包含以下核心组件：

- **插件元数据**：DownloaderPlugin 类实现 IPluginMetadata 接口，提供插件基本信息和本地化资源注册。
- **视图模型基类**：DownloaderViewModelBase 提供通用的脚本执行框架，支持参数配置、日志记录、执行状态管理等。
- **具体视图模型**：M3u8DownloaderViewModel 继承自 DownloaderViewModelBase，实现具体的M3U8下载功能。
- **服务层**：M3u8DownloadService 处理实际的下载逻辑，包括M3U8解析、分片下载、加密解密、FFmpeg合并等。
- **模型层**：包含M3U8解析模型、日志模型、URL条目模型等。
- **UI页面**：M3u8DownloaderPage 提供用户界面，支持多链接下载、参数配置、实时日志显示等。

#### 现代C#特性应用
该插件充分展示了现代C#开发的最佳实践：

- **ObservableObject**：使用 `[ObservableProperty]` 属性自动实现 INotifyPropertyChanged
- **RelayCommand**：使用 `[RelayCommand]` 特性简化命令实现
- **Record Types**：使用 `record` 类型简化不可变数据模型
- **Async/Await**：完整的异步编程模式
- **Dependency Injection**：通过构造函数注入依赖服务

#### 核心功能特性
- **多加密支持**：支持 AES-128、AES-128-ECB、CHACHA20 等加密方式
- **并发下载**：可配置并发数，提高下载效率
- **质量选择**：支持最佳/最差/指定分辨率/带宽选择
- **参数化配置**：支持HTTP请求头、FFmpeg路径、重试次数等参数
- **实时日志**：提供详细的下载进度和错误信息
- **自动合并**：使用FFmpeg自动合并TS分片为MP4文件
...
+ObservableProperty ObservableCollection~ScriptParameter~ Parameters
+ObservableProperty ObservableCollection~LogEntry~ LogEntries
+ObservableProperty bool IsRunning
+ObservableProperty string StatusText
+RelayCommand ExecuteScript()
+RelayCommand CancelExecution()
+abstract ExecuteCoreAsync()
}
class M3u8DownloaderViewModel {
+ObservableProperty string SavePath
+ObservableProperty ObservableCollection~M3u8UrlEntry~ UrlEntries
+RelayCommand AddUrlEntry()
+RelayCommand RemoveUrlEntry()
+RelayCommand BrowseSavePath()
+ExecuteCoreAsync()
}
...
class M3u8DownloaderPage {
+OnDataContextChanged()
+OnLogEntriesCollectionChanged()
}
class DirectUiLogger {
+Log()
}
...
DownloaderPlugin ..> IPluginMetadata : "实现"
DownloaderViewModelBase --|> ViewModelBase : "继承"
M3u8DownloaderViewModel --|> DownloaderViewModelBase : "继承"
M3u8DownloaderViewModel --> M3u8DownloadService : "使用"
M3u8DownloaderViewModel --> DirectUiLogger : "使用"
M3u8DownloaderViewModel --> M3u8Models : "使用"
M3u8DownloaderPage --> M3u8DownloaderViewModel : "绑定"
DirectUiLogger --> LogEntry : "创建"
ScriptParameter --> M3u8DownloaderViewModel : "配置"
```
...
- [M3u8Models.cs:1-10](file://plugins/Avalonia.Plugin.Downloader/Models/M3u8Models.cs#L1-L10)
- [M3u8DownloaderPage.axaml.cs:1-42](file://plugins/Avalonia.Plugin.Downloader/Pages/M3u8DownloaderPage.axaml.cs#L1-L42)
- [DirectUiLogger.cs:1-14](file://plugins/Avalonia.Plugin.Downloader/Services/DirectUiLogger.cs#L1-L14)
- [ScriptParameter.cs:1-121](file://plugins/Avalonia.Plugin.Downloader/Models/ScriptParameter.cs#L1-L121)

**章节来源**
- [DownloaderPlugin.cs:1-24](file://plugins/Avalonia.Plugin.Downloader/DownloaderPlugin.cs#L1-L24)
- [DownloaderViewModelBase.cs:1-128](file://plugins/Avalonia.Plugin.Downloader/ViewModels/DownloaderViewModelBase.cs#L1-L128)
- [M3u8DownloaderViewModel.cs:1-138](file://plugins/Avalonia.Plugin.Downloader/ViewModels/M3u8DownloaderViewModel.cs#L1-L138)
- [M3u8DownloaderPage.axaml:1-160](file://plugins/Avalonia.Plugin.Downloader/Pages/M3u8DownloaderPage.axaml#L1-L160)
- [M3u8DownloaderPage.axaml.cs:1-42](file://plugins/Avalonia.Plugin.Downloader/Pages/M3u8DownloaderPage.axaml.cs#L1-L42)
- [M3u8DownloadService.cs:1-630](file://plugins/Avalonia.Plugin.Downloader/Services/M3u8DownloadService.cs#L1-L630)
- [M3u8Models.cs:1-10](file://plugins/Avalonia.Plugin.Downloader/Models/M3u8Models.cs#L1-L10)
- [LogEntry.cs:1-9](file://plugins/Avalonia.Plugin.Downloader/Models/LogEntry.cs#L1-L9)
- [M3u8UrlEntry.cs:1-52](file://plugins/Avalonia.Plugin.Downloader/Models/M3u8UrlEntry.cs#L1-L52)
- [ScriptParameter.cs:1-121](file://plugins/Avalonia.Plugin.Downloader/Models/ScriptParameter.cs#L1-L121)
- [DirectUiLogger.cs:1-14](file://plugins/Avalonia.Plugin.Downloader/Services/DirectUiLogger.cs#L1-L14)
- [Avalonia.Plugin.Downloader.csproj:1-29](file://plugins/Avalonia.Plugin.Downloader/Avalonia.Plugin.Downloader.csproj#L1-L29)

### 组件五：视图定位器（ViewLocator）
...
**更新** 新增的M3U8下载器插件还依赖 CliWrap 库用于FFmpeg调用，展示了如何在插件中集成第三方库。该插件使用了以下关键依赖：
- CliWrap：用于调用FFmpeg可执行文件进行视频合并
- CommunityToolkit.Mvvm：提供现代化的MVVM特性支持
- System.Security.Cryptography：提供AES和CHACHA20加密算法实现

```mermaid
graph LR
Gen["Avalonia.Plugin.Generators"] --> TPL["Avalonia.Plugin.Template.csproj"]
Gen --> DOWN["Avalonia.Plugin.Downloader.csproj"]
Shared["Avalonia.Plugin.Shared"] --> TPL
Shared --> DOWN
DOWN --> CliWrap["CliWrap"]
DOWN --> CommunityToolkit["CommunityToolkit.Mvvm"]
DOWN --> Crypto["System.Security.Cryptography"]
```
...
- **更新** M3U8下载器插件采用了并发下载策略，通过 SemaphoreSlim 控制最大并发数，平衡下载速度与系统资源占用。同时实现了智能重试机制和内存优化的日志管理。
...
- **更新** M3U8下载器特定问题
  - FFmpeg路径错误：检查FFmpeg可执行文件路径配置，确保FFmpeg在系统PATH中或指定绝对路径。
  - 加密下载失败：确认M3U8文件的加密方式是否受支持，检查网络连接和密钥获取权限。
  - 并发下载超时：调整并发数参数，避免过多并发导致网络拥塞。
  - 内存不足：监控日志队列大小，确保及时清理过时日志条目。
...
## 结论
通过本教程，您已经掌握了 AvaloniaTemplate 插件开发的完整流程：从项目结构与命名约定，到特性声明与接口实现；从视图定位器的工作原理，到导航与菜单的声明式集成；再到性能优化与故障排除。建议以模板插件为起点，逐步扩展到更复杂的业务插件，如M3U8下载器插件所示，遵循共享库提供的契约与最佳实践，确保插件的可维护性与可扩展性。

**更新** 新的M3U8下载器插件展示了现代C#开发的最佳实践，包括使用CommunityToolkit MVVM的ObservableObject、RelayCommand等特性，为开发者提供了现代化的插件开发参考案例。该插件体现了完整的MVVM架构模式，包括数据绑定、命令处理、异步操作和依赖注入等现代C#特性。
...
- 测试与调试
  ...
  - **更新** 对于M3U8下载器插件，建议使用真实M3U8测试链接进行端到端测试，验证多加密支持和并发下载功能。
- 部署
  - 将插件输出复制到宿主应用的插件目录，确保插件清单与依赖满足要求。
  - 使用共享库提供的安装管理器进行安装与卸载操作。
  - **更新** 部署M3U8下载器插件时，确保系统已安装FFmpeg可执行文件，并配置适当的权限以访问网络资源。