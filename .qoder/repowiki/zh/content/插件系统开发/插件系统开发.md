...
## 更新摘要
**变更内容**
- 新增下载器插件重大功能增强章节，详细介绍多链接M3U8下载器功能
- 更新插件架构总览，增加下载器插件的详细说明
- 添加M3u8UrlEntry模型和批量下载支持的实现细节
- 更新依赖分析，包含新的下载器插件组件
- 新增下载器插件开发最佳实践和性能优化建议
...
## 简介
本指南面向使用 AvaloniaTemplate 的插件开发者，系统讲解插件接口设计、元数据与依赖注入支持、插件加载器工作原理、安装与卸载流程，以及从项目创建到部署发布的完整开发流程。文档同时提供最佳实践、常见陷阱与性能优化建议，并给出实用的开发与调试技巧。**更新**：现已涵盖下载器插件的重大功能增强，包括多链接M3U8下载器、批量下载支持和高级加密解密功能。

## 项目结构
AvaloniaTemplate 将插件系统分为三层：
- 共享层（Avalonia.Plugin.Shared）：定义插件接口、元数据模型、服务定位器与通用视图模型。
- 应用层（Avalonia.UI）：实现插件加载、安装、卸载与加载上下文隔离。
- 插件示例（plugins/*）：提供具体插件实现与演示页面，包括新增的下载器插件。
...
- [DownloaderPlugin.cs:8-24](file://plugins/Avalonia.Plugin.Downloader/DownloaderPlugin.cs#L8-L24)
...
### 多链接M3U8下载器功能概述
下载器插件新增了强大的多链接M3U8下载器功能，支持批量下载多个M3U8视频链接，具备以下核心特性：
- **多链接支持**：允许用户同时添加多个M3U8链接进行批量下载
- **智能文件命名**：自动从URL提取文件名，支持自定义文件名覆盖
- **高级加密解密**：支持AES-128、AES-128-ECB、CHACHA20等多种加密方式
- **并发下载**：可配置并发数，提高下载效率
- **FFmpeg集成**：自动合并TS分片为MP4文件
- **详细日志**：实时显示下载进度和状态信息

### M3u8UrlEntry模型设计
新增的M3u8UrlEntry模型是多链接功能的核心，提供了灵活的URL和文件名管理：
...
class EffectiveFileName {
<<property>>
+当FileName为空时，从URL提取文件名
+支持多种URL格式和文件扩展名
}
M3u8UrlEntry --> EffectiveFileName : "计算属性"
```

**图表来源**
- [M3u8UrlEntry.cs:5-52](file://plugins/Avalonia.Plugin.Downloader/Models/M3u8UrlEntry.cs#L5-L52)

### M3u8DownloaderViewModel核心功能
M3u8DownloaderViewModel继承自DownloaderViewModelBase，实现了完整的M3U8下载功能：

- **批量链接管理**：支持动态添加和删除M3U8链接条目
- **参数配置**：提供并发数、画质选择、HTTP头、FFmpeg路径、重试次数等参数
- **保存路径选择**：集成Avalonia存储提供程序，支持文件夹选择对话框
- **执行流程控制**：支持取消操作和状态跟踪

### M3u8DownloadService下载引擎
M3u8DownloadService是下载器的核心服务，实现了完整的M3U8下载流程：

```mermaid
sequenceDiagram
participant VM as "M3u8DownloaderViewModel"
participant S as "M3u8DownloadService"
participant HTTP as "HTTP客户端"
participant FF as "FFmpeg"
VM->>S : "DownloadAsync(url, output, params)"
S->>HTTP : "获取M3U8内容"
HTTP-->>S : "返回master.m3u8"
S->>S : "解析视频流和加密信息"
S->>HTTP : "下载分片TS文件"
S->>S : "解密分片(如需要)"
S->>S : "写入filelist.txt"
...
### UI界面设计
M3u8DownloaderPage提供了直观的用户界面，包含以下主要区域：

- **链接管理区**：支持添加多个M3U8链接，每个链接可自定义文件名
- **保存设置区**：选择保存路径和浏览文件夹
- **参数配置区**：配置下载参数如并发数、画质、HTTP头等
- **日志输出区**：实时显示下载进度和状态信息

### 加密解密支持
下载器支持多种M3U8加密方式：

- **AES-128**：标准AES-128-CBC模式解密
- **AES-128-ECB**：AES-128-ECB模式解密
- **CHACHA20**：ChaCha20流密码解密
- **SAMPLE-AES**：不支持（需要许可证服务器）

### 性能优化特性
- **并发控制**：可配置的并发下载数，平衡速度和资源占用
- **重试机制**：失败分片自动重试，提高成功率
- **内存管理**：及时清理临时文件和释放资源
- **进度报告**：定期更新下载进度和速度统计
...
- 外部依赖：System.Reflection、System.Runtime.Loader、System.Text.Json、Microsoft.CodeAnalysis（生成器）、CliWrap（FFmpeg调用）。
- 循环依赖：无直接循环；生成器仅在编译期参与，运行时不依赖生成结果。
- 接口契约：IPlugin/IPluginMetadata 为弱耦合点，宿主通过接口消费插件能力。
- **新增**：下载器插件依赖CliWrap进行FFmpeg调用，依赖CommunityToolkit进行MVVM模式。
...
Gen --> IPluginMeta
DLVM["M3u8DownloaderViewModel"] --> DLService["M3u8DownloadService"]
DLService --> CliWrap["CliWrap (FFmpeg)"]
DLVM --> DirectUI["DirectUiLogger"]
DLVM --> ScriptParam["ScriptParameter"]
```
...
- [PluginInstallationManager.cs:10-260](file://src/Avalonia.UI/Services/PluginInstallationManager.cs#L10-L260)
- [MetadataGenerator.cs:8-245](file://src/Avalonia.Plugin.Generators/MetadataGenerator.cs#L8-L245)
- [M3u8DownloaderViewModel.cs:14-14](file://plugins/Avalonia.Plugin.Downloader/ViewModels/M3u8DownloaderViewModel.cs#L14-L14)
- [M3u8DownloadService.cs:6-6](file://plugins/Avalonia.Plugin.Downloader/Services/M3u8DownloadService.cs#L6-L6)
...
- **新增**：下载器插件的并发下载优化，合理设置并发数避免网络拥塞。
- **新增**：分片重试机制，提高下载稳定性但避免过度重试影响性能。
- **新增**：临时文件管理，及时清理避免磁盘空间占用。
...
- **新增**：下载器插件问题排查：
  - FFmpeg路径错误：检查ffmpegPath参数配置
  - 网络连接失败：检查HTTP头设置和代理配置
  - 加密解密失败：确认加密方式支持和密钥获取
  - 磁盘空间不足：检查保存路径可用空间
...
## 结论
AvaloniaTemplate 的插件系统以清晰的接口契约、可回收的加载上下文与自动化的源生成为核心，提供了稳定、可扩展且易于维护的插件生态。**更新**：下载器插件的重大功能增强进一步丰富了插件系统的实用性，特别是多链接M3U8下载器功能，为用户提供了完整的视频下载解决方案。遵循本文的开发流程与最佳实践，可高效构建高质量插件并平滑集成到宿主应用中。
