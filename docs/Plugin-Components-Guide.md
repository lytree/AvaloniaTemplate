...
本指南面向插件开发者，列出当前 Host 项目暴露给所有插件的可复用 UI 组件、样式类、服务与基础设施，并给出推荐用法与示例。所有组件均通过 `Avalonia.Plugin.Shared` 与 Host 主题系统自动注入，无需插件主动引用额外 NuGet 包。
...
插件通过 `Avalonia.Plugin.Shared` 的程序集转发机制可访问所有 Ursa 控件。在 XAML 顶部添加：
```xml
...
文件位置：[`src/Avalonia.UI/Theme/FluentDesign/FluentDesignStyles.axaml`](../src/Avalonia.UI/Theme/FluentDesign/FluentDesignStyles.axaml)
...
文件位置：[`src/Avalonia.UI/Theme/Icons/Fluent/`](../src/Avalonia.UI/Theme/Icons/Fluent/)
...
| 接口 | 命名空间 | 用途 |
|------|---------|------|
| `INavigationService` | `Avalonia.UI.Services` | 页面导航 |
| `ILocalizationService` | `Avalonia.Plugin.Shared.Services` | 多语言资源管理 |
| `ISettingsService` | `Avalonia.Plugin.Shared.Services` | 设置读写 |
| `IPluginLoader` | `Avalonia.Plugin.Shared.Services` | 插件加载器 |
| `IPluginInstallationManager` | `Avalonia.Plugin.Shared.Services` | 插件安装管理 |
| `IWindowInfoService` | `Avalonia.Plugin.Shared.Services` | 窗口信息 |
| `ITaskRegistry` | `Avalonia.Plugin.Shared.Services` | 后台任务注册 |
