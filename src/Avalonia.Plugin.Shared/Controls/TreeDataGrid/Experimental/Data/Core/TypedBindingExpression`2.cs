...
namespace Avalonia.Experimental.Data.Core
{
            ...
            where TIn : class
    {
        ...
        private bool SubscribeToChanges(object o)
        {
            ...
            {
                ao.PropertyChanged += (s, e) => ChainPropertyChanged(s);
                result |= true;
            }
            else if (o is INotifyPropertyChanged inpc)
            {
                inpc.PropertyChanged += (s, e) => ChainPropertyChanged(s);
                result |= true;
            }

            if (o is INotifyCollectionChanged incc)
            {
                incc.CollectionChanged += (s, e) => ChainPropertyChanged(s);
                result |= true;
            }
        ...
        {
            // Unsubscription handled by GC when binding expression is released
        }
