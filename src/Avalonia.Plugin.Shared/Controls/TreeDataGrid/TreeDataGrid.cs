    ...
    public class TreeDataGrid : TemplatedControl
    {
        ...
        internal void RaiseRowDragStarted(PointerEventArgs trigger)
        {
            ...
            if (allowedEffects != DragDropEffects.None)
            {
                ...
                _currentDragInfo = info;
                var data = new DataTransfer();
                data.Add(DataTransferItem.CreateText("TreeDataGridDrag"));
                if (trigger is PointerPressedEventArgs pressedArgs)
                    DragDrop.DoDragDropAsync(pressedArgs, data, allowedEffects);
            }
        }
        ...
        {
            if (!AutoDragDropRows ||
                _currentDragInfo is not DragInfo di ||
                _source is null ||
                _source.IsSorted ||
                targetRow is null ||
                di.Source != _source)
